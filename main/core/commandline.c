...
#ifdef WITH_SCREEN
    xTaskCreate(screen_music_visualizer_task, "udp_server", 4096, NULL, 5,
                &VisualizerHandle);
#else
    xTaskCreate(animate_led_based_on_amplitude, "udp_server", 4096, NULL, 5,
                &VisualizerHandle);
#endif
  }
...
void handle_start_portal(int argc, char **argv) {
  ...
  if (argc == 6) {
    ...
    TERMINAL_VIEW_ADD_TEXT(
        "Usage: %s <URL> <SSID> <Password> <AP_ssid> <DOMAIN>\n", argv[0]);
    printf("or\n");
  ...
  if (ssid && ssid[0] != '\0' && password && password[0] != '\0' &&
    ...
    printf("Starting portal in offline mode with AP_SSID: %s, Domain: %s\n",
           ap_ssid, domain);
    TERMINAL_VIEW_ADD_TEXT(
        "Starting portal in offline mode with AP_SSID: %s, Domain: %s\n",
        ap_ssid, domain);
...
void handle_help(int argc, char **argv) {
  ...
  printf("capture\n");
  printf(
      "    Description: Start a WiFi Capture (Requires SD Card or Flipper)\n");
  ...
  TERMINAL_VIEW_ADD_TEXT(
      "    Description: Connects to Specific WiFi Network\n");
  ...
  TERMINAL_VIEW_ADD_TEXT(
      "    Description: Cast a Random Youtube Video on all Smart TV's on your "
  ...
  TERMINAL_VIEW_ADD_TEXT(
      "    Description: Start/Stop BLE wardriving with GPS logging\n");
  ...
  TERMINAL_VIEW_ADD_TEXT(
      "    Description: Scan ports on local subnet or specific IP\n");
  TERMINAL_VIEW_ADD_TEXT(
...
void handle_capture(int argc, char **argv) {
  if (argc < 2) {
    ...
    TERMINAL_VIEW_ADD_TEXT(
        "Usage: capture [-probe|-beacon|-deauth|-raw|-ble]\n");
    return;
  }
#ifndef CONFIG_IDF_TARGET_ESP32S2
...
void handle_gps_info(int argc, char **argv) {
  ...
  if (stop_flag) {
    ...
    if (gps_info_task_handle == NULL) {
      ...
      xTaskCreate(gps_info_display_task, "gps_info", 4096, NULL, 1,
                  &gps_info_task_handle);
