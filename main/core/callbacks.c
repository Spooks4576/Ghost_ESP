...
#define ESP_WIFI_VENDOR_METADATA_LEN                                           \
  8 // Channel(1) + RSSI(1) + Rate(1) + Timestamp(4) + Noise(1)
...
static esp_err_t start_channel_hopping(void) {
  ...
  return esp_timer_start_periodic(channel_hop_timer,
...
static void start_log_task(pineap_network_t *network, const char *new_ssid,
                           int8_t channel, int8_t rssi) {
  ...
  memcpy(log_data->recent_ssids, network->recent_ssids,
         sizeof(network->recent_ssids));
  ...
  // Create logging task
  BaseType_t result = xTaskCreate(log_pineap_detection, "pineap_log", 4096,
                                  log_data, 1, &network->log_task_handle);
  if (result != pdPASS) {
...
static bool is_valid_unique_ssid(const char *new_ssid,
                                 pineap_network_t *network) {
...
void wifi_pineap_detector_callback(void *buf,
  ...
  if (!ssid_hash_exists(network, ssid_hash) &&
      network->ssid_count < MAX_SSIDS_PER_BSSID) {
    ...
    network->recent_ssid_index =
        (network->recent_ssid_index + 1) % RECENT_SSID_COUNT;

    // If we detect multiple SSIDs from same BSSID, mark as potential Pineap
    if (network->ssid_count >= MIN_SSIDS_FOR_DETECTION &&
        (!is_blacklisted(hdr->addr3) ||
         should_update_blacklisted(hdr->addr3))) {
      ...
      if (network->log_task_handle == NULL) {
        ...
        BaseType_t result =
            xTaskCreate(log_pineap_detection, "pineap_log", 4096, log_data, 1,
...
void get_frame_type_and_subtype(const wifi_promiscuous_pkt_t *pkt,
...
void wifi_raw_scan_callback(void *buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (pkt->rx_ctrl.sig_len > 0) {
    esp_err_t ret = pcap_write_packet_to_buffer(
        pkt->payload, pkt->rx_ctrl.sig_len, PCAP_CAPTURE_WIFI);
    if (ret != ESP_OK) {
...
void wardriving_scan_callback(void *buf, wifi_promiscuous_pkt_type_t type) {
  ...
  while (index + 1 < len) {
    ...
    if (id == 48) {
      ...
      uint32_t oui = (payload[index + 2] << 16) | (payload[index + 3] << 8) |
                     payload[index + 4];
  ...
  wardriving_data.ssid[sizeof(wardriving_data.ssid) - 1] =
      '\0'; // Null-terminate
  snprintf(wardriving_data.bssid, sizeof(wardriving_data.bssid),
  ...
  wardriving_data.encryption_type[sizeof(wardriving_data.encryption_type) - 1] =
      '\0';
    ...
    if (id == 221 && ie_len >= 4) {
      ...
      if (oui == 0x0050f2 && oui_type == 0x04) {
        ...
        while (attr_index + 4 <= wps_ie_end) {
          uint16_t attr_id =
              (payload[attr_index] << 8) | payload[attr_index + 1];
          uint16_t attr_len =
              (payload[attr_index + 2] << 8) | payload[attr_index + 3];
          ...
          if (attr_id == 0x1008 && attr_len == 2) {
            ...
            } else if (config_methods & (WPS_CONF_METHODS_PIN_DISPLAY |
            ...
            if (should_store_wps == 1) {
              ...
              new_network.wps_mode =
                  config_methods & (WPS_CONF_METHODS_PIN_DISPLAY |
                                    WPS_CONF_METHODS_PIN_KEYPAD)
                      ? WPS_MODE_PIN
                      : WPS_MODE_PBC;
...
static int ble_hs_adv_parse_fields_cb(const struct ble_hs_adv_field *field,
                                      void *arg);
...
void ble_wardriving_callback(struct ble_gap_event *event, void *arg) {
  ...
  snprintf(wardriving_data.ble_data.ble_mac,
           sizeof(wardriving_data.ble_data.ble_mac),
           "%02x:%02x:%02x:%02x:%02x:%02x", event->disc.addr.val[0],
           event->disc.addr.val[1], event->disc.addr.val[2],
           event->disc.addr.val[3], event->disc.addr.val[4],
  ...
  if (event->disc.length_data > 0) {
    ble_hs_adv_parse(event->disc.data, event->disc.length_data,
                     ble_hs_adv_parse_fields_cb, &wardriving_data);
  }
...
static const int suspicious_names_count =
    sizeof(suspicious_names) / sizeof(suspicious_names[0]);
    ...
    for (int i = 0; i < suspicious_names_count; i++) {
      if (strcasecmp(device_name, suspicious_names[i]) == 0) {
        ...
        if (pcap_file != NULL) {
          ...
          memcpy(enhanced_packet + packet_len, event->disc.data,
                 event->disc.length_data);
