...
static esp_err_t scan_directory(const char *base_path, cJSON *json_array) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    size_t full_path_len =
        strlen(base_path) + strlen(entry->d_name) + 2; // +2 for '/' and '\0'
...
static esp_err_t api_sd_card_get_handler(httpd_req_t *req) {
  ...
  if (stat(base_path, &st) != 0) {
    ...
    httpd_resp_sendstr(
        req, "{\"error\": \"SD card not supported or not mounted.\"}");
    return ESP_FAIL;
  }
  ...
  if (!response_string) {
    ...
    httpd_resp_sendstr(req,
                       "{\"error\": \"Failed to serialize SD card data.\"}");
    return ESP_FAIL;
  }
...
static esp_err_t api_sd_card_post_handler(httpd_req_t *req) {
  ...
  if (!cJSON_IsString(path_item) || !path_item->valuestring) {
    ...
    httpd_resp_set_status(req, "400 Bad Request");
    httpd_resp_sendstr(
        req, "{\"error\": \"'path' is required and must be a string.\"}");
    return ESP_FAIL;
  }
...
esp_err_t get_query_param(httpd_req_t *req, const char *key, char *value,
                          size_t max_len) {
  ...
  if (query_len > 1) { // >1 because query string starts with '?'
    ...
    if (httpd_req_get_url_query_str(req, query, query_len) == ESP_OK) {
      ...
      if (httpd_query_key_value(query, key, encoded_value,
                                sizeof(encoded_value)) == ESP_OK) {
...
esp_err_t api_sd_card_delete_file_handler(httpd_req_t *req) {
  ...
  if (query_len > 1) {
    ...
    if (httpd_query_key_value(query, "path", path, sizeof(path)) == ESP_OK) {
      ...
      if (res == 0) {
        ...
        httpd_resp_send(req, "File deleted successfully",
                        HTTPD_RESP_USE_STRLEN);
        return ESP_OK;
      } else {
        ESP_LOGE(TAG, "Failed to delete file: %s, errno: %d", filepath, errno);
        httpd_resp_set_status(req, "500 Internal Server Error");
        httpd_resp_send(req, "Failed to delete the file",
                        HTTPD_RESP_USE_STRLEN);
        return ESP_FAIL;
      }
    }
  ...
  httpd_resp_set_status(req, "400 Bad Request");
  httpd_resp_send(req, "Missing or invalid 'path' parameter",
                  HTTPD_RESP_USE_STRLEN);
  return ESP_FAIL;
}
...
static esp_err_t api_sd_card_upload_handler(httpd_req_t *req) {
  ...
  if (get_query_param(req, "path", path_param, sizeof(path_param)) != ESP_OK) {
    ...
    httpd_resp_set_type(req, "application/json");
    httpd_resp_sendstr(
        req, "{\"error\": \"Missing or invalid 'path' query parameter.\"}");
    return ESP_FAIL;
  }
  ...
  size_t boundary_len =
      strlen(boundary_start) + 3; // +3 for "--" and null terminator
  ...
  FILE *file = NULL;
  char *file_path =
      malloc(MAX_PATH_LENGTH + 128); // Allocate heap memory for file_path
  if (!file_path) {
      ...
      if (strstr(boundary_ptr,
        ...
        if (filename_start) {
          ...
          if (filename_end &&
            ...
            strncpy(original_filename, filename_start,
        ...
        if (strlen(original_filename) > 0) {
          ...
          snprintf(file_path, MAX_PATH_LENGTH + 128, "%s/received_file",
                   path_param);
        }
        ...
        if (data_len > 0 &&
            fwrite(headers_end, 1, data_len, file) != data_len) {
  ...
  ESP_LOGI(TAG, "File uploaded successfully: %zu bytes received.",
           total_received);
...
esp_err_t ap_manager_init(void) {
  ...
  if (ap_netif == NULL) {
    ...
    ip_info.gw.addr =
        ESP_IP4TOADDR(192, 168, 4, 1); // Gateway (usually same as IP)
  ...
  ESP_ERROR_CHECK(esp_event_handler_register(
      IP_EVENT, IP_EVENT_AP_STAIPASSIGNED, &event_handler, NULL));
...
void ap_manager_stop_services() {
  ...
  ESP_ERROR_CHECK(esp_event_handler_unregister(
      IP_EVENT, IP_EVENT_AP_STAIPASSIGNED, &event_handler));
...
static esp_err_t api_settings_handler(httpd_req_t *req) {
  ...
  if (portal_offline_mode) {
    settings_set_portal_offline_mode(settings,
                                     portal_offline_mode->valueint != 0);
  }
  ...
  if (printer_alignment) {
    ...
    settings_set_printer_alignment(
        settings, (PrinterAlignment)printer_alignment->valueint);
  }
  ...
  if (display_timeout) {
    ...
    ESP_LOGI(TAG, "Setting display timeout to: %d ms",
             display_timeout->valueint);
  }
...
static esp_err_t api_settings_get_handler(httpd_req_t *req) {
  ...
  cJSON_AddNumberToObject(root, "broadcast_speed",
                          settings_get_broadcast_speed(settings));
  cJSON_AddStringToObject(root, "ap_ssid", settings_get_ap_ssid(settings));
  cJSON_AddStringToObject(root, "ap_password",
                          settings_get_ap_password(settings));
  ...
  cJSON_AddNumberToObject(root, "channel_delay",
                          settings_get_channel_delay(settings));

  cJSON_AddStringToObject(root, "portal_url",
                          settings_get_portal_url(settings));
  cJSON_AddStringToObject(root, "portal_ssid",
                          settings_get_portal_ssid(settings));
  cJSON_AddStringToObject(root, "portal_password",
                          settings_get_portal_password(settings));
  cJSON_AddStringToObject(root, "portal_ap_ssid",
                          settings_get_portal_ap_ssid(settings));
  cJSON_AddStringToObject(root, "portal_domain",
                          settings_get_portal_domain(settings));
  cJSON_AddBoolToObject(root, "portal_offline_mode",
                        settings_get_portal_offline_mode(settings));

  cJSON_AddStringToObject(root, "printer_ip",
                          settings_get_printer_ip(settings));
  cJSON_AddStringToObject(root, "printer_text",
                          settings_get_printer_text(settings));
  cJSON_AddNumberToObject(root, "printer_font_size",
                          settings_get_printer_font_size(settings));
  cJSON_AddNumberToObject(root, "printer_alignment",
                          settings_get_printer_alignment(settings));
  cJSON_AddStringToObject(root, "hex_accent_color",
                          settings_get_accent_color_str(settings));
  cJSON_AddStringToObject(root, "timezone_str",
                          settings_get_timezone_str(settings));
  cJSON_AddNumberToObject(root, "gps_rx_pin",
                          settings_get_gps_rx_pin(settings));
  cJSON_AddNumberToObject(root, "display_timeout",
                          settings_get_display_timeout(settings));
  cJSON_AddNumberToObject(root, "rts_enabled_bool",
                          settings_get_rts_enabled(settings));
