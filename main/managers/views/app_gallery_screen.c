...
void apps_menu_create(void) {
  ...
  static lv_coord_t row_dsc[] = {LV_GRID_FR(0.5), // Row 0: Back button
  ...
  lv_obj_set_grid_cell(back_button, LV_GRID_ALIGN_CENTER, 1, 1,
  ...
  lv_obj_set_grid_cell(next_button, LV_GRID_ALIGN_CENTER, 2, 1,
                       LV_GRID_ALIGN_START, 3, 1);
  lv_obj_set_user_data(
      next_button,
      (void *)(uintptr_t)(-2)); // Use -2 to identify the next button
  ...
  lv_obj_set_grid_cell(prev_button, LV_GRID_ALIGN_CENTER, 0, 1,
                       LV_GRID_ALIGN_START, 3, 1);
  lv_obj_set_user_data(
      prev_button,
      (void *)(uintptr_t)(-3)); // Use -3 to identify the prev button
...
void refresh_apps_menu(void) {
  // Delete existing app items except the back button and next/prev buttons (if
  // touchscreen)
  ...
  for (int i = start_index; i < end_index; i++) {
    ...
    lv_obj_set_grid_cell(app_item, LV_GRID_ALIGN_CENTER, display_index % 3, 1,
...
#ifndef CONFIG_USE_TOUCHSCREEN
  // For devices without touchscreens, ensure selected_app_index is within the
  // current page
  if (selected_app_index != -1 &&
      (selected_app_index < start_index || selected_app_index >= end_index)) {
...
void apps_menu_event_handler(InputEvent *event) {
  if (event->type == INPUT_TYPE_TOUCH) {
    ...
    if (touched_app_index >= 0) {
      ...
      printf("Touch input detected but no app item found at touch coordinates. "
             "X: %d, Y: %d\n",
             data->point.x, data->point.y);
    }
