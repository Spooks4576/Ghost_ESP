...
void apps_menu_create(void) {
  ...
  // Determine orientation (should be landscape for 1024x600)
  ...
  // Grid layout: 3 columns across width, rows down height
  ...
  // Button sizes for landscape (7-inch screen, e.g., 1024x600)
  int button_width = LV_HOR_RES / 4;  // e.g., 256 pixels
  int button_height = LV_VER_RES / 8; // e.g., 75 pixels
  ...
  lv_obj_set_style_pad_column(apps_container, 10, 0); // Positive spacing between columns
  ...
  lv_obj_set_style_pad_top(apps_container, 5, 0); // Small margin from top
  ...
  // Place Back button in bottom-left corner
  #ifdef CONFIG_USE_TOUCHSCREEN
  ...
  // Refresh app items (ensure apps populate from top)
  refresh_apps_menu();
...
void refresh_apps_menu(void) {
  // Clear existing app items (preserve navigation buttons)
  uint32_t child_count = lv_obj_get_child_cnt(apps_container);
  ...
  // Button sizes for landscape (e.g., 1024x600)
  int button_width = LV_HOR_RES / 4;  // e.g., 256 pixels
  int button_height = LV_VER_RES / 4; // e.g., 150 pixels
  int icon_width = 50;
  int icon_height = 50;
  ...
  // Populate app buttons starting at row 0 (top)
  for (int i = start_index; i < end_index; i++) {
    int display_index = i - start_index;
    if (display_index >= 3) break; // Limit to 3 buttons for now
    ...
    // Place in row 0, columns 0-2 (top row)
    int row_idx = display_index / 3; // Start at row 0
    int col_idx = display_index % 3; // Columns 0, 1, 2
