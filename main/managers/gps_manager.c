...
void gps_manager_init(GPSManager *manager) {
  ...
  xTaskCreate(check_gps_connection_task, "gps_check", 2048, NULL, 1,
...
esp_err_t gps_manager_log_wardriving_data(wardriving_data_t *data) {
  ...
  if (!is_valid_date(&gps->date)) {
    ...
    if (gps->valid && gps->fix >= GPS_FIX_GPS && gps->fix_mode >= GPS_MODE_2D &&
        gps->sats_in_use >= 3 &&
        gps->sats_in_use <= GPS_MAX_SATELLITES_IN_USE && rand() % 100 == 0) {
      ESP_LOGW(GPS_TAG,
               "Invalid date despite good fix: %04d-%02d-%02d "
               "(Fix: %d, Mode: %d, Sats: %d)",
               gps_get_absolute_year(gps->date.year), gps->date.month,
               gps->date.day, gps->fix, gps->fix_mode, gps->sats_in_use);
    }

    // Use cached date for validation
    ESP_LOGD(GPS_TAG, "Using cached date: %04d-%02d-%02d",
             gps_get_absolute_year(cacheddate.year), cacheddate.month,
             cacheddate.day);
  ...
  if (!is_valid_date(&gps->date)) {
    ...
    if (gps->valid && gps->fix >= GPS_FIX_GPS && gps->fix_mode >= GPS_MODE_2D &&
        ...
        rand() % 100 == 0) {
      printf(
          "Warning: GPS date is out of range despite good fix: %04d-%02d-%02d "
          "(Fix: %d, Mode: %d, Sats: %d)\n",
  ...
  if (rand() % GPS_UPDATE_INTERVAL == 0) {
    ...
    const char *fix_status = (!gps->valid || gps->fix == GPS_FIX_INVALID)
                                 ? "No Fix"
    ...
    float speed_kmh = 0.0;
    if (gps->valid &&
        gps->fix >= GPS_FIX_GPS) { // Only trust speed with a valid fix
      if (gps->speed >= MIN_SPEED_THRESHOLD &&
          gps->speed <= MAX_SPEED_THRESHOLD) {
    ...
    printf("\n");
    printf(GPS_STATUS_MESSAGE, fix_status, data->gps_quality.satellites_used,
           GPS_MAX_SATELLITES_IN_USE,
           data->gps_quality.speed * 3.6, // Convert m/s to km/h
           get_gps_quality_string(
               data)); // Only keep the arguments that match the format string
    TERMINAL_VIEW_ADD_TEXT(
