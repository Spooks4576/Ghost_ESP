...
static bool extract_company_id(const uint8_t *payload, size_t length,
                               uint16_t *company_id) {
  size_t index = 0;
...
static void parse_device_name(const uint8_t *data, uint8_t data_len, char *name,
                              size_t name_size) {
  int index = 0;
...
static void parse_service_uuids(const uint8_t *data, uint8_t data_len,
                                ble_service_uuids_t *uuids) {
  int index = 0;
  ...
  while (index < data_len) {
    ...
    if ((type == BLE_HS_ADV_TYPE_COMP_UUIDS16 ||
    ...
    else if ((type == BLE_HS_ADV_TYPE_COMP_UUIDS32 ||
    ...
    // Check for 128-bit UUIDs
      ...
      snprintf(uuids->uuid128[uuids->uuid128_count],
               ...
               "02x%02x",
               data[index + 17], data[index + 16], data[index + 15],
               data[index + 14], data[index + 13], data[index + 12],
               data[index + 11], data[index + 10], data[index + 9],
               data[index + 8], data[index + 7], data[index + 6],
               data[index + 5], data[index + 4], data[index + 3],
               data[index + 2]);
      uuids->uuid128_count++;
    }
...
void ble_findtheflippers_callback(struct ble_gap_event *event, size_t len) {
  ...
  snprintf(advertisementMac, sizeof(advertisementMac),
  ...
  parse_device_name(event->disc.data, event->disc.length_data,
                    advertisementName, sizeof(advertisementName));
  ...
  for (int i = 0; i < uuids.uuid16_count; i++) {
    if (uuids.uuid16[i] == 0x3082) {
      ...
      TERMINAL_VIEW_ADD_TEXT("Found White Flipper Device (128-bit UUID): "
                             "\nMAC: %s, \nName: %s, \nRSSI: %d\n",
                             advertisementMac, advertisementName,
                             advertisementRssi);
...
void detect_ble_spam_callback(struct ble_gap_event *event, size_t length) {
  ...
  if (last_company_id != NULL && *last_company_id == current_company_id) {
    ...
    if (spam_counter > MAX_PAYLOADS) {
      ESP_LOGW(TAG_BLE, "BLE Spam detected! Company ID: 0x%04X",
               current_company_id);
...
esp_err_t ble_unregister_handler(ble_data_handler_t handler) {
  for (int i = 0; i < handler_count; i++) {
    if (handlers[i].handler == handler) {
      ...
      handler_count--;
      ble_handler_t *new_handlers =
          realloc(handlers, handler_count * sizeof(ble_handler_t));
...
void ble_init(void) {
  ...
  if (!ble_initialized) {
    ...
    xTaskCreateStatic(nimble_host_task, "nimble_host",
                      sizeof(host_task_stack) / sizeof(StackType_t), NULL, 5,
                      host_task_stack, &host_task_buf);
...
static void ble_pcap_callback(struct ble_gap_event *event, size_t len) {
  ...
  if (event->type == BLE_GAP_EVENT_DISC) {
    ...
    uint8_t param_len =
        10 + event->disc.length_data; // 1 (subevent) + 1 (num reports) + 1
