...
static esp_err_t st7789_init(void) {
    ESP_LOGI(TAG, "Initializing SPI bus for ST7789");
    ...
    spi_bus_config_t bus_config = {
        ...
        .max_transfer_sz = 240 * 320 * 2 + 10, // Maximum transfer size (adjust for panel size)
    };
    ESP_ERROR_CHECK(spi_bus_initialize(SPI2_HOST, &bus_config, SPI_DMA_CH_AUTO));

    
    ESP_LOGI(TAG, "Setting up panel IO for ST7789");

    esp_lcd_panel_io_spi_config_t io_config = {
        .dc_gpio_num = BOARD_TFT_DC,         // Data/command pin
        .cs_gpio_num = BOARD_TFT_CS,         // Chip select pin
        .pclk_hz = 40 * 1000 * 1000,         // Pixel clock frequency (adjust as needed)
        ...
        .on_color_trans_done = NULL,         // (Optional) Callback for transaction done event
    };
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        ...
        .rgb_endian = LCD_RGB_ENDIAN_RGB,    // RGB color order (can be BGR or RGB)
    ...
    ESP_LOGI(TAG, "Additional configurations for ST7789 panel");
    ...
    ESP_LOGI(TAG, "Turning on the ST7789 display");
    ESP_ERROR_CHECK(esp_lcd_panel_disp_on_off(panel_handle, true));

    
    ESP_LOGI(TAG, "Turning on the backlight for ST7789");
    gpio_config_t bk_gpio_config = {
        .mode = GPIO_MODE_OUTPUT,
        .pin_bit_mask = 1ULL << BOARD_TFT_BL,  // Backlight pin
    };
    ESP_ERROR_CHECK(gpio_config(&bk_gpio_config));
    gpio_set_level(BOARD_TFT_BL, 1); // Turn on backlight (1 for on, 0 for off)
    return ESP_OK;
}

static esp_err_t st7789_draw_bitmap(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const void *data) {
    return esp_lcd_panel_draw_bitmap(panel_handle, x, y, x + width, y + height, data);
}

static esp_err_t st7789_invert_color(bool invert) {
    return esp_lcd_panel_invert_color(panel_handle, invert);
}

static esp_err_t st7789_set_gap(uint16_t x_gap, uint16_t y_gap) {
    return esp_lcd_panel_set_gap(panel_handle, x_gap, y_gap);
}

static esp_err_t st7789_mirror(bool mirror_x, bool mirror_y) {
    return esp_lcd_panel_mirror(panel_handle, mirror_x, mirror_y);
}

static esp_err_t st7789_swap_xy(bool swap_axes) {
    return esp_lcd_panel_swap_xy(panel_handle, swap_axes);
}

static esp_err_t st7789_disp_on_off(bool on_off) {
    return esp_lcd_panel_disp_on_off(panel_handle, on_off);
}

// Instantiate the driver
display_driver_t st7789_driver = {
    .init = st7789_init,
